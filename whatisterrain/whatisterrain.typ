...
#show: setup.with(
  top: 27.4mm,
  bottom: 27.4mm,
  outer: (far: 24.8mm, width: 47.7mm, sep: 6.2mm), 
  book: true
)
...
Defining what a 'terrain' is, also called a _digital terrain model_ (DTM), is not a simple task because there is no universal agreement, neither among practitioners nor in the scientific literature. Different terms are used, often interchangeably.
...
However, the "Earth's surface" is also not a clear concept, since several objects on it can be present, e.g.~man-made structures like buildings, roads, power lines, and bridges, and other objects like trees.

#place(bottom,
  float: true, 
  wideblock(
    [#figure(
      image("figs/destm.pdf"),
      placement: auto,
...
/ DTM: *d*\igital *t*\errain *m*\odel#index[digital terrain model (DTM)]: the surface is the _bare-earth_, without man-made objects or vegetation. 
/ DSM: *d*\igital *s*\urface *m*\odel)#index[digital surface model (DSM)]: the surface includes all objects and structures on the terrain, including vegetation. 
/ DEM: *d*\igital *e*\levation *m*\odel)#index[digital elevation model (DEM)]:   in the literal meaning of the term, it is simply a model of the elevation. A DEM is either a DSM or a DTM. #note[DEM $eq.not$ grid] However, it should be noticed that in some countries (especially in the USA) a DEM is often synonymous with a grid of elevation (but not in this book!). 

Furthermore, some concepts are derived from those, for instance:

/ nDSM (*n*\ormalised DSM): the difference between the DSM and the DTM. #index[normalised DSM (nDSM)] We thus obtain a value representing the height of man-made objects and trees, and zero where there are none.
/ CHM (*c*\anopy *h*\eight *m*\odel): similarly to the nDSM except that we are only interested in the vegetation/trees; we ignore the other objects (e.g. buildings, bridges, lampposts, busstops, cars, etc.). #index[canopy height model (CHM)] The CHM is the difference between the DSM _of the vegetation_ and the DTM.
...
What is usually used for modelling terrains: a surface (which is topologically a 2D object; also called a 2-manifold) is embedded in 3D space, and each location ($x,y$) is assigned to one and only one height $z$.
...
=== Full 3D, or volumetric modelling
...
#index[Euclidian space] #note [Euclidian space]
...
Fields must therefore be _discretised_, i.e.~broken into finite parts.
...
To represent a terrain in a computer, and be able to manipulate it (i.e.~edit the terrain and extract information such as slope), two things are needed:
...
This operation is referred to as spatial interpolation (see @chap:interpol and @chap:kriging). 
...
In general, we  classify the tessellations of space into three categories (as shown in @fig:tesstypes): _regular_, _irregular_, and _hierarchical_. #index[tessellation]
...
== Data models for representing terrains in a computer
// TODO Data models for terrains]{Data models for representing terrains in a computer}[Data models for terrains]

=== Spatial data models $eq.not$ data structures
...
As shown in @fig:tesstypes\a, all the cells have the same shape and size.
...
#showybox(
  frame: (
    title-color: rgb("#F7E0E0"),
    body-color:  rgb("#F7E0E0"),
    radius: 0pt,
    dash: none, 
    thickness: 0pt
  ),
  ...
  title: [#fa-icon("gear") Concrete example: a 2D grid], 
...
Second, grids scale badly and are not rotationally invariant, i.e.~if the coordinate reference system used is changed, then the grid needs to be reconstructed to obtain regular cells whose boundaries are parallel to the axes of the reference system.
...
#notefigure(
  image("figs/tin.pdf"), 
  caption: [A TIN is obtained by lifting the vertices to their elevation. All the triangles are usually Delaunay, i.e.~their circumcircle (green) is empty of any other points in the plane.]
) <fig:tin>
...
The main reason is that Delaunay triangles are as "fat" as possible (long and skinny triangles are avoided), and thus they behave better for interpolation.
 ...
 ) <fig:whydt> // TODO look at caption settings
the estimated value can be significantly different, and in this case the right one would make more sense since sample points that are closer to the interpolation location are used (in the TIN on the left, the value of #unify.qty(95, "m") is not used).
...
A TIN in which a linear interpolation function is used yields a $C^0$ piecewise representation, i.e.~it is a continuous function but at the edges of the triangles the first derivative is not possible.
It is possible to use higher-order functions in each triangle of a TIN, to construct a $C^1$ or $C^2$ field, i.e.~where the first and second derivative of the surface can be obtained. 
...
// TODO rewrite: zero or 4 children? 
...
The main advantage of using them---saving memory space---is present only when there is spatial coherence between cells having the same attribute value, i.e.~when they are clustered together.
Indeed, the size of a quadtree is not dependent on the number of cells, but on their distribution.
The quadtree of a 2D grid having no two adjacent cells with the same value (e.g.~a checkers board) contains the same number of cells as the grid, and its size would most likely be worse because of the overhead to manage the tree.
...
+ irregularly spaced sample points, such as a point cloud; 
+ contour lines.
...
Given a bivariate field $f(x, y) = z$, an _isoline_ (commonly referred to as contour line) is the set of points in space where $f(x,y) = z_0$, where $z_0$ is a constant. 
...
#figure(
  ...
  caption: [An example of a terrain (top) and its contour lines at every #unify.qty(100, "m") (bottom).]
) <fig:contours>
...
Another property that follows from the $2.5 D$ property of the field is that contours neither intersect themselves nor each other.
...
#figure(
  ...
  caption: [Cross-section of a terrain (left), and the #unify.qty(200, "metre") isoline extracted from a TIN representation of it (right).]
) <fig:isoline>
the idea is to compute the intersection between the level value (e.g.~#unify.qty(200, "metre")) and the terrain, represented for instance with a TIN. 
...
== TIN versus raster for modelling terrains
// TODO [TIN versus raster]
...
It should be said that both TIN and raster have advantages and disadvantages (as we will see during this course), and in practice one should choose the most appropriate model for the task at hand. // TODO rephrase: "throughout this book"?
...
#citet(<Frank92>) #citet(<Goodchild92a>) discuss at length the issue of data model, data structure and representation of reality. 
...
+ Explain in your own words why a point cloud (e.g.~ collected with airborne lidar) is not considered a complete representation of a terrain.
